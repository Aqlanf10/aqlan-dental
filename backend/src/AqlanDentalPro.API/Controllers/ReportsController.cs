...
public class ReportsController(AppDbContext db, IPdfService pdfService, ILogger<ReportsController> logger, ICurrentUserService currentUser) : ControllerBase
{
    ...
    public async Task<IActionResult> GetCenterSummary([FromQuery] string? from, [FromQuery] string? to)
    {
        ...
        var (fromDate, fromErr) = DateParsingHelper.TryParseDateOrDefault(from, ClinicTimeProvider.ClinicToday().AddDays(-30), "تاريخ البداية");
    ...
    {
        var (fromDate, fromErr) = DateParsingHelper.TryParseDateOrDefault(from, ClinicTimeProvider.ClinicToday().AddDays(-365), "تاريخ البداية");
        if (fromErr != null) return fromErr;
        var (toDate, toErr) = DateParsingHelper.TryParseDateOrDefault(to, ClinicTimeProvider.ClinicToday(), "تاريخ النهاية");
        if (toErr != null) return toErr;
    ...
    {
        var (fromDate, fromErr) = DateParsingHelper.TryParseDateOrDefault(from, ClinicTimeProvider.ClinicToday().AddDays(-90), "تاريخ البداية");
        if (fromErr != null) return fromErr;
        var (toDate, toErr) = DateParsingHelper.TryParseDateOrDefault(to, ClinicTimeProvider.ClinicToday(), "تاريخ النهاية");
        if (toErr != null) return toErr;
    ...
    {
        var (reportDate, dateErr) = DateParsingHelper.TryParseDateOrDefault(date, ClinicTimeProvider.ClinicToday(), "التاريخ");
