...
interface CephAiDraftResponse { draft: string; modelId?: string; disclaimer?: string; generatedAt?: string; }
...
const STEPS: WizardStep[] = [
  ...
  { ...s(3, "extraoral", "الصور خارج الفم", "أمامي، جانبي، ابتسامة", "الحد الأدنى للعرض: صورة أمامية، بروفايل، وابتسامة في طور البداية.", "التقط الصور مباشرة من الجوال، ثم جهّزها للعرض بالقص والتدوير إن احتاجت.", Camera, "photos", "critical", undefined, ["ExtraoralPhotos"]), photoSlots: [
  ...
  { ...s(5, "intraoral", "الصور داخل الفم", "أمامي، يمين، يسار، علوي، سفلي", "الحد الأدنى السريري: أمامي، يمين، يسار، إطباقي علوي، إطباقي سفلي.", "استخدم كاميرا الجوال الخلفية، ثم جهّز الصور بنسب عرض مناسبة دون تشويه.", Scan, "photos", "critical", undefined, ["IntraoralPhotos"]), photoSlots: [
...
function s(order: number, key: string, title: string, required: string, minimum: string, doctorAction: string, icon: typeof User, kind: WizardStep["kind"], priority: StepPriority, tab?: Tab, slideTypes?: string[]): WizardStep {
...
export function OrthoCaseWizard({ caseId, patientId, onNavigate }: { caseId: string; patientId?: string; onNavigate: (tab: Tab) => void }) {
  ...
  const latestCephAnalysisId = typeof o.latestCephAnalysisId === "string" ? o.latestCephAnalysisId : null;
  ...
  const photosFor = (slot: PhotoSlot) => photos.find((photo) => (eq(photo.category, slot.category) || eq(photo.photoType, slot.category)) && eq(photo.subtype, slot.subtype) && (slot.phase === "Initial" ? !photo.treatmentPhase || eq(photo.treatmentPhase, "Initial") : eq(photo.treatmentPhase, slot.phase)));
  ...
  const generate = async (includeEmpty: boolean) => {
    if (!includeEmpty && !canGenerateFinal) { toast.error("لا يمكن إنشاء عرض نهائي قبل إكمال الخطوات الأساسية الناقصة"); if (firstBlocking) setOpen(firstBlocking.step.key); return; }
    setBusy(true);
    try {
    ...
    } catch { toast.error("تعذّر إنشاء العرض"); } finally { setBusy(false); }
  };

  return (
    <div className="space-y-4" dir="rtl">
      <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><h3 className="flex items-center gap-2 text-base font-bold text-clinic-navy"><Presentation className="h-5 w-5 text-clinic-blue" />معالج عرض الحالة الموجّه</h3><p className="mt-1 max-w-3xl text-xs leading-6 text-gray-500">اتبع نفس تسلسل عرض الحالة: بيانات → صور → فحص وتحاليل → قائمة مشاكل → تشخيص → أهداف واستراتيجيات → خطة وميكانيكا → توليد.</p></div><div className="flex flex-wrap gap-2"><span className={cn("rounded-full px-3 py-1 text-xs font-bold", canGenerateFinal ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700")}>{readyCount}/{totalSteps} خطوة جاهزة</span>{criticalMissing.length > 0 && <button type="button" onClick={() => setOpen(criticalMissing[0].step.key)} className="inline-flex items-center gap-1 rounded-full bg-red-50 px-3 py-1 text-xs font-bold text-red-700 hover:bg-red-100"><ArrowUpCircle className="h-3.5 w-3.5" />أول نقص أساسي: {criticalMissing[0].step.title}</button>}</div></div>
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-3"><SummaryCard label="خطوات أساسية ناقصة" value={criticalMissing.length} tone={criticalMissing.length ? "danger" : "success"} /><SummaryCard label="خطوات جزئية" value={partialSteps.length} tone={partialSteps.length ? "warning" : "success"} /><SummaryCard label="الجاهزية النهائية" value={canGenerateFinal ? "جاهز" : "غير جاهز"} tone={canGenerateFinal ? "success" : "warning"} /></div>
      </div>

      <ol className="space-y-2">
        {steps.map(({ step, status }) => {
          const meta = STATUS_META[status]; const priority = PRIORITY_META[step.priority]; const Icon = step.icon; const StatusIcon = meta.icon; const isOpen = open === step.key; const captured = step.photoSlots?.filter((slot) => photosFor(slot)).length ?? 0; const totalSlots = step.photoSlots?.length ?? 0;
          return <li key={step.key} className={cn("overflow-hidden rounded-xl border bg-white shadow-sm", step.priority === "critical" && !isReady(status) ? "border-red-200" : "border-gray-200")}>
            ...
            {isOpen && <div className="border-t border-gray-100 bg-gray-50/50 p-3"><StepGuidance step={step} status={status} />
              ...
              {step.kind === "data" && step.tab && <div className="mt-3 space-y-3">{step.draftKind && <ClinicalDraftPanel caseId={caseId} step={step} latestCephAnalysisId={latestCephAnalysisId} />}<div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => onNavigate(step.tab!)} className="inline-flex items-center gap-1.5 rounded-lg bg-clinic-blue px-3 py-1.5 text-xs font-medium text-white hover:opacity-90"><Pencil className="h-3.5 w-3.5" />فتح شاشة الإدخال المرتبطة</button>{step.draftKind && <span className="inline-flex items-center gap-1 rounded-lg border border-dashed border-clinic-blue/40 bg-white px-2.5 py-1.5 text-[11px] text-gray-600"><Sparkles className="h-3.5 w-3.5 text-clinic-blue" />AI هنا مسودة فقط ولا يعتمد شيئًا تلقائيًا</span>}</div></div>}
              {step.kind === "photos" && step.photoSlots && <div className="mt-3 space-y-2"><div className="rounded-lg border border-blue-100 bg-blue-50 p-2 text-[11px] leading-5 text-blue-800">التقط الصورة من نفس هذه الخطوة حتى تُحفظ تلقائيًا بالفئة والطور الصحيحين. بعد الحفظ اضغط «قص وتجهيز». الأصل يبقى محفوظًا كما هو.</div><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{step.photoSlots.map((slot) => <PhotoSlotCard key={slot.key} caseId={caseId} slot={slot} photo={photosFor(slot) ?? null} disabled={busy} onChanged={refresh} onPrepare={(photo) => setPrepPhoto(photo)} />)}</div></div>}
              {step.kind === "generate" && <div className="mt-3 space-y-3"><div className={cn("rounded-lg border p-3", canGenerateFinal ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50")}><p className="text-xs font-semibold text-gray-700">الجاهزية النهائية: {readyCount}/{totalSteps}</p>{criticalMissing.length > 0 ? <ul className="mt-2 grid gap-1 sm:grid-cols-2">{criticalMissing.map((item) => <li key={item.step.key} className="flex items-center gap-1.5 text-[11px] text-amber-800"><AlertCircle className="h-3.5 w-3.5" />ناقص أساسي: {item.step.title}</li>)}</ul> : <p className="mt-1 text-[11px] text-green-800">كل الخطوات الأساسية مكتملة. يمكن إنشاء عرض نهائي بدون شرائح فارغة.</p>}</div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => generate(false)} disabled={busy || !canGenerateFinal} className="inline-flex items-center gap-1.5 rounded-lg bg-clinic-blue px-3 py-1.5 text-xs font-bold text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">{busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Presentation className="h-3.5 w-3.5" />}إنشاء عرض نهائي بلا شرائح فارغة</button><button type="button" onClick={() => generate(true)} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50">مسودة تعليمية تشمل الفارغة</button>{!canGenerateFinal && <span className="self-center text-[11px] text-amber-700">العرض النهائي مقفل حتى تكتمل الخطوات الأساسية.</span>}</div></div>}
            </div>}
          </li>;
        })}
      </ol>
...
function SummaryCard({ label, value, tone }: { label: string; value: string | number; tone: "success" | "warning" | "danger" }) { const cls = tone === "success" ? "border-green-100 bg-green-50 text-green-800" : tone === "danger" ? "border-red-100 bg-red-50 text-red-800" : "border-amber-100 bg-amber-50 text-amber-800"; return <div className={cn("rounded-lg border p-2", cls)}><div className="text-[11px] opacity-80">{label}</div><div className="text-sm font-bold">{value}</div></div>; }
...
function ClinicalDraftPanel({ caseId, step, latestCephAnalysisId }: { caseId: string; step: WizardStep; latestCephAnalysisId: string | null }) {
  ...
  const generateAiDraft = async () => {
    if (!latestCephAnalysisId) { toast.error("لا يوجد تحليل سيفالو محفوظ لاستخدام مساعد AI النصي"); return; }
    setAiLoading(true);
    try {
      const { data } = await api.post<CephAiDraftResponse>(`/api/ceph/${latestCephAnalysisId}/ai/draft-diagnosis`);
      const generated = [
        `AI draft from latest cephalometric analysis${data.modelId ? ` (${data.modelId})` : ""}:`,
        data.draft,
        data.disclaimer,
      ].filter(Boolean).join("\n\n");
      setDraft((current) => current.trim() && current.trim() !== template.trim() ? `${current.trim()}\n\n---\n${generated}` : generated);
      toast.success("تم توليد مسودة AI — راجعها قبل الاعتماد");
    } catch (error) {
      const message = (error as { response?: { data?: { message?: string } } }).response?.data?.message ?? "تعذر توليد مسودة AI";
      toast.error(message);
    } finally { setAiLoading(false); }
  };

  return <div className="rounded-xl border border-clinic-blue/20 bg-white p-3 shadow-sm"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-1.5 text-xs font-bold text-clinic-navy"><Clipboard className="h-3.5 w-3.5 text-clinic-blue" />مسودة سريرية داخل المعالج</div><p className="mt-1 text-[11px] leading-5 text-gray-500">اكتب تفكيرك هنا بنفس ترتيب العرض. يمكنك طلب مسودة AI حقيقية من آخر تحليل سيفالو إن وُجد، لكنها لا تُحفظ ولا تُعتمد تلقائيًا.</p></div><div className="flex flex-wrap gap-1.5"><button type="button" onClick={generateAiDraft} disabled={aiLoading || !latestCephAnalysisId} className="inline-flex items-center gap-1 rounded-lg bg-clinic-blue px-2.5 py-1.5 text-[11px] font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">{aiLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}اقتراح AI</button><button type="button" onClick={copy} className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1.5 text-[11px] font-medium text-gray-700 hover:bg-gray-50"><Clipboard className="h-3.5 w-3.5" />نسخ</button></div></div><textarea value={draft} onChange={(event) => setDraft(event.target.value)} rows={8} className="mt-3 w-full rounded-lg border border-gray-200 bg-gray-50 p-3 text-xs leading-6 text-gray-800 outline-none focus:border-clinic-blue focus:bg-white" />{!latestCephAnalysisId && <div className="mt-2 rounded-lg bg-gray-50 p-2 text-[11px] leading-5 text-gray-600">زر AI يتطلب وجود تحليل سيفالو محفوظ للحالة. بدون ذلك تبقى المسودة اليدوية متاحة.</div>}<div className="mt-2 rounded-lg bg-amber-50 p-2 text-[11px] leading-5 text-amber-800">مسودة AI مبنية أساسًا على السيفالو والفحص المرتبطين بالحالة. لا تعتمدها ولا تدخلها في التقرير إلا بعد مراجعة الطبيب.</div></div>;
}
...
function draftTemplate(kind?: WizardStep["draftKind"]) {
  switch (kind) {
    ...
    case "mechano": return "Mechanotherapy sequence:\n\nPhase 1 — Alignment and leveling:\n- \n\nPhase 2 — Space closure / correction:\n- \n\nPhase 3 — Finishing and detailing:\n- \n\nElastics / TADs / auxiliaries:\n- ";
    default: return "Clinical draft:\n- ";
  }
}

function PhotoSlotCard({ caseId, slot, photo, disabled, onChanged, onPrepare }: { caseId: string; slot: PhotoSlot; photo: OrthoPhoto | null; disabled: boolean; onChanged: () => void; onPrepare: (p: OrthoPhoto) => void }) {
  ...
  const onFile = async (file: File | undefined) => { if (!file) return; setUploading(true); try { const form = new FormData(); form.append("file", file); const { data } = await api.post<{ url: string }>("/api/uploads", form, { headers: { "Content-Type": "multipart/form-data" } }); await orthoService.addPhoto(caseId, { photoUrl: data.url, category: slot.category, subtype: slot.subtype, photoType: slot.category, treatmentPhase: slot.phase, isSelectedForReport: true, caption: slot.label } as Partial<OrthoPhoto>); toast.success("تم حفظ الصورة في مكانها الصحيح"); onChanged(); } catch { toast.error("تعذّر رفع الصورة"); } finally { setUploading(false); if (inputRef.current) inputRef.current.value = ""; } };
  return <div className="rounded-lg border border-gray-200 bg-white p-2.5"><div className="flex items-center justify-between gap-2"><div><span className="block text-xs font-semibold text-gray-700">{slot.label}</span><span className="text-[10px] text-gray-400">{slot.phase === "Final" ? "صورة نهائية" : "صورة بداية"}</span></div>{photo ? <span className="inline-flex items-center gap-1 rounded-full bg-green-50 px-1.5 py-0.5 text-[10px] text-green-700"><CheckCircle2 className="h-3 w-3" />محفوظة</span> : <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500">مطلوبة</span>}</div>{photo ? <img src={resolveImageUrl(preparedUrl ?? photo.photoUrl)} alt={slot.label} className="mt-2 h-28 w-full rounded-md object-cover" /> : <div className="mt-2 grid h-28 place-items-center rounded-md border border-dashed border-gray-200 bg-gray-50 text-[11px] text-gray-400">لم تُلتقط بعد</div>}<input ref={inputRef} type="file" accept="image/*" capture={slot.capture} className="hidden" onChange={(event) => onFile(event.target.files?.[0])} /><div className="mt-2 flex flex-wrap gap-1.5"><button type="button" disabled={disabled || uploading} onClick={() => inputRef.current?.click()} className="inline-flex items-center gap-1 rounded-md bg-clinic-blue px-2.5 py-1 text-[11px] font-medium text-white hover:opacity-90 disabled:opacity-50">{uploading ? <Loader2 className="h-3 w-3 animate-spin" /> : <Camera className="h-3 w-3" />}{photo ? "إعادة الالتقاط" : "التقاط / رفع"}</button>{photo && <button type="button" onClick={() => onPrepare(photo)} className="inline-flex items-center gap-1 rounded-md border border-gray-200 px-2.5 py-1 text-[11px] font-medium text-gray-600 hover:bg-gray-50"><Upload className="h-3 w-3" />قص وتجهيز</button>}{preparedUrl && <span className="inline-flex items-center gap-1 rounded-md bg-blue-50 px-2.5 py-1 text-[11px] text-blue-700"><BadgeCheck className="h-3 w-3" />جاهزة للعرض</span>}</div></div>;
}
