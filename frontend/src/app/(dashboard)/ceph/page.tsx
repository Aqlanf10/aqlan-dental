                ...
                {analyses.map((a) => {
                  ...
                  return (
                    <tr key={a.id} className={cn("transition", isBase ? "bg-blue-50/60" : isSameCase ? "bg-emerald-50/40 hover:bg-emerald-50/70" : "hover:bg-gray-50", compareBase && !isBase && !isSameCase && "opacity-50")}>
                      ...
                      <td className="px-4 py-3"><div className="flex items-center gap-2"><span className={cn("inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-bold", landmarkReady ? "bg-green-50 text-green-700" : a.landmarkCount > 0 ? "bg-amber-50 text-amber-700" : "bg-gray-100 text-gray-500")}><Ruler className="h-3.5 w-3.5" />{a.landmarkCount}/24</span>{a.aiAssisted && <span title="مساعدة الذكاء الاصطناعي"><Brain className="w-3.5 h-3.5 text-purple-500" /></span>}</div></td>
