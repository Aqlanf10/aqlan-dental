...
- `CEPH-REQ-030`: Every cephalometric AI inference SHALL retain immutable model/preprocessing/dataset/definition/artifact identity and original predictions separately from doctor corrections. Promotion and rollback SHALL require recorded validation, statistics, and clinical approvals and SHALL NOT relabel historic analyses or imply measured accuracy.
- `CEPH-REQ-031`: The blinded cephalometric Pilot SHALL pin `ADP-LM-LAT-v1.0` per project, remain separate from clinical patient records, use two distinct assigned reviewers, hide AI/WebCeph/other-reviewer output server-side, and stage only de-identified account-owned exports in private storage. No Pilot entity SHALL contain `PatientId`; no patient SHALL be created automatically; WebCeph SHALL remain a comparator, and the 24-point Gold Standard SHALL require later explicit independent review and adjudication without automatic averaging.
