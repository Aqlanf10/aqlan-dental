...
public class MessagingService(AppDbContext db, ICurrentUserService currentUser, INotificationService notifications, IRealTimePushService pushService, IPatientAccountLinkingService linkingService) : IMessagingService
{
    ...
    public async Task<MessageDto> SendMessageAsync(Guid conversationId, SendMessageRequest request)
    {
        ...
        // before a previous operation completed." This was the root cause of
        // "فشل إرسال الرسالة" errors when conversations had multiple participants.
        ...
        foreach (var pid in otherParticipants)
        {
            try
            {
                await pushService.PushToUserAsync(pid, "UnreadCountUpdated", new { conversationId });
            }
            catch (Exception)
            {
                // Non-critical
