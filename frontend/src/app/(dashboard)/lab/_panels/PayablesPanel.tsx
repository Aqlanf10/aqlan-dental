...
import type { Treasury } from "@/app/(dashboard)/finance-v3/components/types";
...
function RecordPaymentModal({
  ...
  // CORE-LAB-014: this screen used to post `paymentMethod: "cash"` unconditionally and never
  // send a treasury, even though the endpoint accepts both — so a bank transfer to a lab was
  // booked as cash out of whichever treasury the server happened to resolve. Every other money
  // dialog in the system lets the user choose; this one was the outlier.
  const [paymentMethod, setPaymentMethod] = useState("cash");
  const [treasuryId, setTreasuryId] = useState("");

  const { data: treasuries = [], isLoading: treasuriesLoading } = useQuery({
    queryKey: ["treasuries", "active"],
    queryFn: async () => {
      const res = await api.get<{ data: Treasury[] }>("/api/finance-v3/treasuries");
      return res.data?.data ?? [];
    },
  });

  // Default to a treasury holding this payable's currency — paying a SAR lab bill out of a
  // YER drawer is the mistake this picker exists to prevent.
  useEffect(() => {
    if (treasuryId || treasuries.length === 0) return;
    const match = treasuries.find((t) => t.currency === payable.currency) ?? treasuries[0];
    setTreasuryId(match.id);
  }, [treasuries, treasuryId, payable.currency]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* CORE-LAB-014: method and treasury are chosen, not assumed. */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700">الصرف من الخزينة *</label>
              {treasuriesLoading ? (
                <p className="text-xs text-gray-400 py-2">جاري تحميل الخزائن...</p>
              ) : treasuries.length === 0 ? (
                <p className="text-xs text-red-600 py-2">لا توجد خزائن نشطة — أنشئ خزينة أولاً</p>
              ) : (
                <select
                  value={treasuryId}
          ...
          {/* A visible warning beats a silent mismatch: paying a SAR bill from a YER drawer
              is legal here but almost always a mistake, so it is named rather than blocked. */}
          {treasuryId && treasuries.find((t) => t.id === treasuryId)?.currency !== payable.currency && (
            <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-3 py-2">
              عملة الخزينة المختارة تختلف عن عملة المستحق ({payable.currency}) — تأكد أن هذا مقصود.
            </p>
          )}
...
/* ─── Main Page ──────────────────────────────────────────────────────────────── */
export function LabPayablesPanel() {
