    ...
    IAuditService audit) : ControllerBase
{
            ...
            try
            {
                ...
                var order = new LabOrder
                {
                    ...
                    SentDate      = hasExplicitSentDate
                        ? DateOnly.TryParse(req.SentDate, out var sentDate) ? sentDate : DateOnly.FromDateTime(ClinicTimeProvider.ClinicToday()) : null,
    ...
    public async Task<IActionResult> UpdateStatus(Guid id, [FromBody] UpdateLabOrderStatusRequest req)
    {
        ...
        if (nextStatus == "sent" && order.SentDate is null)
            order.SentDate = DateOnly.FromDateTime(ClinicTimeProvider.ClinicToday());
        if (nextStatus == "delivered")
            order.DeliveredDate = DateOnly.FromDateTime(ClinicTimeProvider.ClinicToday());
