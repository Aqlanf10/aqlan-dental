...
export function PhotosTab({ patientId }: PhotosTabProps) {
  ...
  return (
    <div className="space-y-4" dir="rtl">
      {/* Add photo form */}
      <form onSubmit={handleAddPhoto} className="bg-[#f7fafd] rounded-lg p-4 border border-[#e8f0f9] space-y-3">
        <p className="text-xs font-semibold text-[#64748b] mb-1">إضافة صورة</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="sm:col-span-2">
            ...
            <label className={cn(
              ...
              photoFile ? "border-clinic-blue bg-clinic-blue-50" : "border-[#e8f0f9] hover:border-[#c8d8e8]"
            )}>
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-[#94a3b8]">JPG، PNG، WebP — حتى 10 ميجابايت</p>
              </div>
            </label>
          </div>
          <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {photos.map((photo) => (
            <div key={photo.id} className="flex items-start gap-3 bg-white border border-[#e8f0f9] rounded-lg p-3 hover:border-[#e8f0f9] transition">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                  ...
                  {photo.photoType && (
                    <span className="text-xs text-[#64748b]">{photo.photoType}</span>
                  )}
                  {photo.stage && (
                    <span className="text-xs bg-[#3d7ab518] text-[#3d7ab5] px-1.5 py-0.5 rounded">
                ...
                <p className="text-xs text-[#94a3b8] mt-0.5">{photo.photoDate}</p>
                {photo.notes && <p className="text-xs text-[#64748b] mt-0.5 line-clamp-1">{photo.notes}</p>}
              </div>
              <button
