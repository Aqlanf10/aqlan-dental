...
import {
  FRONTAL_LANDMARKS, computeFrontalMeasurements, frontalSummary,
  type FrontalSeverity, type FrontalMeasurement,
} from "@/lib/photoAnalysisFrontal";
...
interface Props {
  ...
  onChange?: (data: { points: Record<string, Pt>; measurements: FrontalMeasurement[] }) => void;
}

const SEVERITY = {
  normal: { icon: CheckCircle, bg: "bg-green-50 text-green-700", label: "طبيعي" },
  mild: { icon: AlertTriangle, bg: "bg-amber-50 text-amber-700", label: "خفيف" },
  severe: { icon: XCircle, bg: "bg-red-50 text-red-700", label: "واضح" },
} as const;

export function FrontalPhotoAnalyzer({ imageUrl, initialPoints, onChange }: Props) {
  const svgRef = useRef<SVGSVGElement>(null);
  const [dim, setDim] = useState<{ w: number; h: number } | null>(null);
  const [points, setPoints] = useState<Record<string, Pt>>(initialPoints ?? {});
  const [selected, setSelected] = useState<string>(FRONTAL_LANDMARKS[0].key);
  ...
  const measurements = useMemo(() => computeFrontalMeasurements(points), [points]);
  const summary = frontalSummary(measurements);

  useEffect(() => { onChange?.({ points, measurements }); }, [points, measurements, onChange]);
  const r = dim ? Math.max(4, dim.w / 140) : 6;
  ...
  const placeAt = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const order = FRONTAL_LANDMARKS.map((l) => l.key);
      ...
      <div className="lg:col-span-2 bg-slate-900 rounded-xl border border-gray-200 overflow-hidden">
        {dim ? (
          <svg
            ...
            onClick={placeAt}
            role="img"
            aria-label="تحليل صورة الوجه الأمامية"
          >
            ...
            {Object.entries(points).map(([key, pt]) => (
              <g key={key}>
                ...
                <text x={pt.x + r * 1.4} y={pt.y - r * 0.6} fontSize={r * 2}
      ...
      <div className="space-y-3">
        <div className="bg-white rounded-xl border border-gray-200 p-3">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-bold text-gray-700">معالم الوجه الأمامية</h3>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
          <h3 className="text-sm font-bold text-gray-700">النسب والتناظر</h3>
          {summary && (
          ...
          {measurements.map((m) => {
            const cfg = SEVERITY[m.severity as FrontalSeverity];
            const Icon = cfg.icon;
            return (
              <div key={m.key} className="border-b border-gray-100 last:border-0 pb-2 last:pb-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-sm font-bold font-mono text-gray-900" dir="ltr">
                    {m.value !== null ? m.value.toFixed(2) : <span className="text-gray-300 text-xs">—</span>}
                  </span>
                </div>
          ...
          <p className="text-[9px] text-gray-400 leading-relaxed pt-1">
            نِسَب هندسية مستقلة عن المقياس (لا تتطلب معايرة) — أداة مساعدة سريرية تتطلب مراجعة الأخصائي.
          </p>
        </div>
      </div>
    </div>
