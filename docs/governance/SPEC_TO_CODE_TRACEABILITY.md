...
| `CEPH-REQ-027` | Cephalometry gold standard | Strict de-identified manifest, patient split isolation, two-reviewer evidence, explicit adjudication | `CephBenchmarkManifestValidator`, `CephBenchmarkController`, benchmark JSON schema | `CephBenchmarkManifestTests`; documentation contract; CI/deployment | complete in PR #691; no accuracy claim | 2026-07-14 | Critical |
| `CEPH-REQ-028` | Cephalometry landmark evaluation | Pinned non-training core-landmark metrics, failures, patient-cluster bootstrap, privacy-safe strata | `CephLandmarkEvaluationEngine`, `CephBenchmarkController`, evaluation DTO/schema | `CephLandmarkEvaluationEngineTests`; ceph suite; docs/encoding; CI/deployment | implementation active in SEQ-54 | 2026-07-14 | Critical |
| `CEPH-REQ-029` | Cephalometry advanced evaluation | Shared geometry, derived measurements, repeatability, confidence, comparator deltas | `backend/src/AqlanDentalPro.Application/Services/CephLateralGeometryEngine.cs`; `backend/src/AqlanDentalPro.Application/Services/CephMeasurementEvaluationEngine.cs`; `backend/src/AqlanDentalPro.Application/Services/CephRepeatabilityEvaluationEngine.cs`; `backend/src/AqlanDentalPro.API/Controllers/CephBenchmarkController.cs`; `docs/ceph-ai/CEPH_AI_VALIDATION_PROTOCOL.md` | frozen geometry, deterministic measurement/Bland-Altman/paired bootstrap, three-run displacement/ICC, confidence-coverage fixtures, strict API/schema, and full ceph regression suites; independent statistics/clinical review still required | SEQ-55 implementation in progress; clinical evidence unmeasured | 2026-07-14 | Critical |
| `CEPH-REQ-030` | Cephalometry model lineage | Immutable model/preprocessing/dataset/definition/artifact identity, original inference output, correction linkage, evidence-gated pin/rollback | `CephAiModelVersion`; `CephAiInferenceRun`; `CephAiModelDeployment`; `CephAiModelRegistryService`; `CephAiModelsController`; `CephLandmark.SourceInferenceRunId`; model-version schema | registry/inference/save-lineage fixtures; focused/full ceph tests; migration review; docs/encoding; CI/deployment | SEQ-56 implementation active; no model promoted and accuracy unmeasured | 2026-07-15 | Critical |
| `CEPH-REQ-031` | Cephalometry blinded Pilot foundation | Ratified v1.0 registration, patient-independent project/case staging, private account-owned export storage, distinct assigned reviewers, server authorization | `CephPilotProject`; `CephPilotCase`; `CephPilotExportArtifact`; `CephPilotController`; `CephPilotStateTransitions`; `docs/ceph-ai/ratified/` | Pilot state/auth/upload/privacy tests; additive migration review; full ceph regression; encoding/build/CI/deployment | SEQ-57 PR-1 active; annotation and comparator evaluation intentionally deferred | 2026-07-16 | Critical |
