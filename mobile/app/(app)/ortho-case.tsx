...
export default function OrthoCaseScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (caseResult.status === "fulfilled") {
      ...
      setOrthoCase(null);
      setError(
        caseResult.reason instanceof Error ? caseResult.reason.message : "تعذر تحميل حالة التقويم"
      );
    }
    ...
    if (visitsResult.status === "fulfilled") {
      ...
      setVisits([]);
      setVisitError(
        visitsResult.reason instanceof Error ? visitsResult.reason.message : "تعذر تحميل زيارات التقويم"
      );
    }
    setLoading(false);
  }, [allowed, id]);
  ...
  if (!allowed) {
    return (
      <Screen>
        <StateMessage title="غير مصرح" message="هذه الوحدة مخصصة لأخصائي التقويم والإدارة." />
      </Screen>
    );
  }
    ...
    <Screen refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} />}>
      ...
      <Card>
        ...
        <Row
          ...
          value={orthoCase.expectedDurationMonths ? `${orthoCase.expectedDurationMonths} شهر` : "—"}
          last={!orthoCase.totalFee}
        />
        {orthoCase.totalFee != null ? (
          <Row label="القيمة المسجلة للحالة" value={orthoCase.totalFee.toLocaleString()} last />
        ) : null}
      </Card>
      ...
      <View style={styles.progressTrack}>
        <View
          style={[
            styles.progressFill,
            { width: `${Math.max(0, Math.min(100, orthoCase.stagePercentage))}%` }
          ]}
        />
      </View>
          ...
          onPress={() =>
            router.push({
              ...
              params: {
                ...
                currentStage: orthoCase.currentStage ?? "",
                doctorId: orthoCase.doctorId ?? ""
              }
            })
          }
        />
          ...
          <Card key={stage.id}>
            <View style={styles.stageHeader}>
              <View style={styles.stageBadge}>
                <Text style={styles.stageBadgeText}>{STAGE_STATUS_LABELS[stage.status] ?? stage.status}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.stageName}>{stage.stageOrder}. {stage.stageName}</Text>
                {stage.targetDurationMonths ? (
                  <Text style={styles.meta}>المدة المستهدفة: {stage.targetDurationMonths} شهر</Text>
                ) : null}
              </View>
            </View>
        ...
        <>
          ...
          <Card>
            {orthoCase.extractionDecisionValue ? (
              <Row label="قرار الخلع" value={orthoCase.extractionDecisionValue} />
            ) : null}
            {orthoCase.retentionPlan ? (
              <Row label="خطة التثبيت" value={orthoCase.retentionPlan} last />
            ) : null}
          </Card>
        </>
      ) : null}
      ...
      {visitError ? (
        <StateMessage title="تعذر تحميل زيارات التقويم" message={visitError} />
      ) : visits.length === 0 ? (
        <StateMessage title="لا توجد زيارات تقويمية مسجلة" />
      ) : (
        visits.map((visit) => (
          <Card key={visit.id}>
            <View style={styles.visitHeader}>
              <Text style={styles.visitDate}>{visit.visitDate}</Text>
              <Text style={styles.visitNumber}>زيارة #{visit.visitNumber}</Text>
            </View>
            ...
            {visit.nextAppointmentDate ? (
              <Text style={styles.next}>الموعد القادم: {visit.nextAppointmentDate}{visit.nextAppointmentType ? ` • ${visit.nextAppointmentType}` : ""}</Text>
            ) : null}
          </Card>
        ))
      )}
