...
- `CEPH-TASK-021`: SEQ-55 geometry and advanced evaluation — extract the canonical lateral geometry into one versioned engine and add derived-measurement, repeatability/ICC, paired comparator, confidence/coverage, and clinical-tolerance evaluation with backend/frontend parity fixtures. — 🟡 Frozen geometry was merged in PR #693 and calculators in PR #694 with all CI/deployment checks green; independent statistics/clinical review, tolerance approval, and the locked study remain. Clinical accuracy remains unmeasured.
- `CEPH-TASK-022`: SEQ-56 model and inference lineage — persist immutable model/preprocessing/dataset/definition/artifact identities, append-oriented inference attempts and original normalized predictions, correction-to-run linkage, and evidence-gated Admin pin/rollback while preserving the unsaved-draft clinical workflow. — 🟡 Implementation active on `codex/seq-56-ceph-model-lineage`; no model promotion or accuracy claim.
- `CEPH-TASK-023`: SEQ-57 Pilot foundation — register the ratified `ADP-LM-LAT-v1.0` manual and approval hashes; add additive project/case/export-staging entities; require distinct real-user reviewer assignments; enforce assignment-scoped reads; strip image metadata by server-side re-encoding; keep images and official WebCeph exports in private storage; expose no patient link, storage key, or image hash; and stage WebCeph artifacts as unreviewed comparator evidence only. PR-1 excludes annotation sessions, comparison, adjudication, manifest release, AI benchmark execution, and patient migration. — 🟡 Implementation active on `codex/seq-57-ceph-pilot-foundation`; no patient data or accuracy claim.
