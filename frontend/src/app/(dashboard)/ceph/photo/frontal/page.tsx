...
import { ArrowRight, ImagePlus, Loader2, Smile } from "lucide-react";
...
function FrontalPhotoPageInner() {
  ...
  return (
    <div className="space-y-5 max-w-5xl" dir="rtl">
      <div className="no-print flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <span className="text-gray-900 font-medium">تحليل الصورة الأمامية</span>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-extrabold text-gray-900 flex items-center gap-2">
            <Smile className="w-6 h-6 text-clinic-blue" />
            تحليل الصورة الأمامية (النسب والتناظر)
          </h1>
        </div>
        {imageUrl && (
          <button
      ...
      {!imageUrl ? (
        <button
          ...
          <span className="text-sm font-medium">
            {uploading ? "جارٍ رفع الصورة..." : "ارفع صورة وجه أمامية (JPG/PNG/WEBP)"}
          </span>
          <span className="text-[11px] text-gray-300">النِسَب مستقلة عن المقياس لا تتطلب معايرة</span>
        </button>
      ) : (
        <FrontalPhotoAnalyzer imageUrl={imageUrl} />
      )}
    </div>
  );
}
