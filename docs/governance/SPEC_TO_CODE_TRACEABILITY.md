...
| `CEPH-REQ-029` | Cephalometry advanced evaluation | Shared geometry, derived measurements, repeatability, confidence, comparator deltas | `backend/src/AqlanDentalPro.Application/Services/CephLateralGeometryEngine.cs`; `backend/src/AqlanDentalPro.Infrastructure/Services/CephService.cs`; `docs/ceph-ai/CEPH_AI_VALIDATION_PROTOCOL.md` | frozen-geometry unit and full ceph regression suites; parity/statistics/clinical fixtures still required | SEQ-55 geometry sub-step in progress | 2026-07-14 | Critical |
