...
public partial class AddFinancialIntegrityAuditSprint : Migration
{
    protected override void Up(MigrationBuilder migrationBuilder)
    {
        ...
        // C3: Create FKs for reversal columns (idempotent — use DO $$ block)
        migrationBuilder.Sql(
            @"DO $$ BEGIN
                IF NOT EXISTS (
        ...
        // PostgreSQL uses xid (xmin) for row versioning, but EF Core byte[] concurrency
        // needs a bytea column. We use bytea with DEFAULT NULL.
        migrationBuilder.Sql(
