    ...
    ILogger<FinanceV3Controller> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> GetDashboard([FromQuery] string? period = "today")
    {
        ...
        try
        {
            ...
            var todayInflow = 0m; var todayOutflow = 0m; var monthInflow = 0m; var monthOutflow = 0m;
            try
            {
                var todayInflowQuery = db.CashFlowTransactions.Where(t => t.TransactionDate == today && t.Type == TransactionType.Inflow);
                var todayOutflowQuery = db.CashFlowTransactions.Where(t => t.TransactionDate == today && t.Type == TransactionType.Outflow);
                var monthInflowQuery = db.CashFlowTransactions.Where(t => t.TransactionDate >= monthStart && t.Type == TransactionType.Inflow);
                var monthOutflowQuery = db.CashFlowTransactions.Where(t => t.TransactionDate >= monthStart && t.Type == TransactionType.Outflow);
                ...
                todayInflow = await todayInflowQuery.SumAsync(t => (decimal?)t.Amount) ?? 0;
                todayOutflow = await todayOutflowQuery.SumAsync(t => (decimal?)t.Amount) ?? 0;
                monthInflow = await monthInflowQuery.SumAsync(t => (decimal?)t.Amount) ?? 0;
                monthOutflow = await monthOutflowQuery.SumAsync(t => (decimal?)t.Amount) ?? 0;
            }
            catch (Exception ex) { logger.LogWarning(ex, "Dashboard: failed to compute cash KPIs, returning zeros"); }

            // ── Outstanding balances ──
            var contractOutstanding = 0m; var invoiceOutstanding = 0m;
            try
            {
                contractOutstanding = await CalculateContractOutstandingAsync(branchId);
                invoiceOutstanding = await CalculateInvoiceOutstandingAsync(branchId);
            }
            catch (Exception ex) { logger.LogWarning(ex, "Dashboard: failed to compute outstanding balances, returning zeros"); }

            // ── Journal Entry stats (canonical verification) ──
            var journalEntryCount = 0; var postedEntryCount = 0; var reversalEntryCount = 0;
            try
            {
                journalEntryCount = await db.JournalEntries.CountAsync(e => !branchId.HasValue || e.BranchId == branchId.Value);
                postedEntryCount = await db.JournalEntries.CountAsync(e => e.IsPosted && (!branchId.HasValue || e.BranchId == branchId.Value));
                reversalEntryCount = await db.JournalEntries.CountAsync(e => e.IsReversal && (!branchId.HasValue || e.BranchId == branchId.Value));
            }
            catch (Exception ex) { logger.LogWarning(ex, "Dashboard: failed to compute journal entry stats, returning zeros"); }

            // ── Treasury summary ──
            var totalTreasuryBalance = 0m;
            try
            {
                ...
                totalTreasuryBalance = await treasuryQuery.SumAsync(t => (decimal?)t.Balance) ?? 0;
            }
            catch (Exception ex) { logger.LogWarning(ex, "Dashboard: failed to compute treasury balance, returning zero"); }

            // ── Accrued revenue from posted JournalLines ──
            var todayAccruedRevenue = 0m; var monthAccruedRevenue = 0m;
            try
            {
                todayAccruedRevenue = await db.JournalLines
                    .Where(l => l.AccountType == JournalAccountType.Revenue
                        && l.JournalEntry != null && l.JournalEntry.EntryDate == today
                        && l.JournalEntry.IsPosted
                        && (!branchId.HasValue || l.BranchId == branchId.Value))
                    .SumAsync(l => (decimal?)(l.Credit - l.Debit)) ?? 0;
                monthAccruedRevenue = await db.JournalLines
                    .Where(l => l.AccountType == JournalAccountType.Revenue
                        && l.JournalEntry != null && l.JournalEntry.EntryDate >= monthStart
                        && l.JournalEntry.IsPosted
                        && (!branchId.HasValue || l.BranchId == branchId.Value))
                    .SumAsync(l => (decimal?)(l.Credit - l.Debit)) ?? 0;
            }
            catch (Exception ex) { logger.LogWarning(ex, "Dashboard: failed to compute accrued revenue, returning zeros"); }

            // ── Pending approvals ──
            var pendingExpenses = 0; var pendingTransfers = 0;
            try
            {
                ...
                if (branchId.HasValue)
                {
                    ...
                    pendingTransfersQuery = pendingTransfersQuery.Where(t => t.DestinationTreasury != null && t.DestinationTreasury.BranchId == branchId.Value);
                }
                pendingExpenses = await pendingExpensesQuery.CountAsync();
                pendingTransfers = await pendingTransfersQuery.CountAsync();
            }
            catch (Exception ex) { logger.LogWarning(ex, "Dashboard: failed to compute pending approvals, returning zeros"); }

            return Ok(new
            {
                TodayInflow, TodayOutflow, TodayNet = todayInflow - todayOutflow,
                MonthInflow, MonthOutflow, MonthNet = monthInflow - monthOutflow,
                TotalOutstanding = contractOutstanding + invoiceOutstanding,
                ContractOutstanding = contractOutstanding, InvoiceOutstanding = invoiceOutstanding,
                TotalTreasuryBalance, TodayAccruedRevenue, MonthAccruedRevenue,
                JournalEntryCount = journalEntryCount, PostedEntryCount = postedEntryCount, ReversalEntryCount = reversalEntryCount,
                DualWriteCoverage = journalEntryCount > 0 ? $"{(double)postedEntryCount / journalEntryCount * 100:F1}%" : "N/A",
                PendingExpenses = pendingExpenses, PendingTransfers = pendingTransfers,
                Date = today.ToString("yyyy-MM-dd"), Period = period
            });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Dashboard: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل لوحة المعلومات" });
        }
    }
        ...
        [FromQuery] bool? isReversal = null)
    {
        ...
        {
            logger.LogError(ex, "GetJournalEntries: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    public async Task<IActionResult> GetJournalEntryById(Guid id)
    {
        ...
        {
            logger.LogError(ex, "GetJournalEntryById: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    public async Task<IActionResult> GetAccountBalances()
    {
        ...
        {
            logger.LogError(ex, "GetAccountBalances: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    public async Task<IActionResult> GetDailyCashSummary([FromQuery] string? date = null)
    {
        ...
        try
        {
            ...
            // PaymentMethod group-by can have null PaymentMethod
            object? byPaymentMethodRaw = null;
            try
            {
                byPaymentMethodRaw = await transactions
            ...
            catch (Exception ex) { logger.LogWarning(ex, "GetDailyCashSummary: failed to group by PaymentMethod, returning empty"); }
            ...
            return Ok(new
            {
                ...
                ByPaymentMethod = byPaymentMethodRaw ?? Array.Empty<object>(),
        ...
        {
            logger.LogError(ex, "GetDailyCashSummary: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? toDate = null)
    {
        ...
        try
        {
            ...
            var accruedRevenueQuery = db.JournalLines
                .Where(l => l.AccountType == JournalAccountType.Revenue
                    && l.JournalEntry != null && l.JournalEntry.EntryDate >= from && l.JournalEntry.EntryDate <= to
                    && l.JournalEntry.IsPosted
                    && (!branchId.HasValue || l.BranchId == branchId.Value));
        ...
        {
            logger.LogError(ex, "GetProfitAndLoss: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    public async Task<IActionResult> GetPatientBalance(Guid patientId)
    {
        ...
        {
            logger.LogError(ex, "GetPatientBalance: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    public async Task<IActionResult> GetTreasuries()
    {
        ...
        {
            logger.LogError(ex, "GetTreasuries: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? action = null)
    {
        ...
        {
            logger.LogError(ex, "GetAuditTrail: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? search = null)
    {
        ...
        {
            logger.LogError(ex, "GetPatientAccounts: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    public async Task<IActionResult> GetTrialBalance([FromQuery] string? asOfDate = null)
    {
        ...
        try
        {
            ...
            var linesQuery = db.JournalLines
                .Where(l => l.JournalEntry != null && l.JournalEntry.IsPosted && l.JournalEntry.EntryDate <= cutoff)
                .Where(l => !branchId.HasValue || l.BranchId == branchId.Value);
        ...
        {
            logger.LogError(ex, "GetTrialBalance: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    public async Task<IActionResult> GetActiveCashierSession()
    {
        ...
        {
            logger.LogError(ex, "GetActiveCashierSession: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? toDate = null)
    {
        ...
        {
            logger.LogError(ex, "GetPayments: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? toDate = null)
    {
        ...
        {
            logger.LogError(ex, "GetInvoices: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? status = null)
    {
        ...
        {
            logger.LogError(ex, "GetContracts: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] int pageSize = 30)
    {
        ...
        {
            logger.LogError(ex, "GetSuppliers: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? status = null)
    {
        ...
        {
            logger.LogError(ex, "GetSupplierBills: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? status = null)
    {
        ...
        try
        {
            ...
            if (branchId.HasValue)
                query = query.Where(t => t.DestinationTreasury != null && t.DestinationTreasury.BranchId == branchId.Value);
        ...
        {
            logger.LogError(ex, "GetVaultTransfers: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
        ...
        [FromQuery] string? approvalStatus = null)
    {
        ...
        {
            logger.LogError(ex, "GetExpenses: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    public async Task<IActionResult> GetActiveCashierSessionV3()
    {
        try
        {
            ...
            return Ok(new
            {
                ...
                CashierName = session.Cashier != null ? session.Cashier.Username : "",
                session.BranchId,
        ...
        {
            logger.LogError(ex, "GetActiveCashierSessionV3: unexpected error");
            return StatusCode(500, new { message = "حدث خطأ في تحميل البيانات" });
        }
    }
    ...
    private async Task<decimal> CalculateContractOutstandingAsync(Guid? branchId)
    {
        ...
        {
            logger.LogError(ex, "CalculateContractOutstandingAsync: unexpected error");
            return 0m;
        }
    }
    ...
    private async Task<decimal> CalculateInvoiceOutstandingAsync(Guid? branchId)
    {
        ...
        {
            logger.LogError(ex, "CalculateInvoiceOutstandingAsync: unexpected error");
            return 0m;
        }
    }
}
